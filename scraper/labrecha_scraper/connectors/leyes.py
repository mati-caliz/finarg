...
class LeyesConnector(Connector):
    name = "leyes"
    source = "hcdn"

    def fetch(self) -> LeyesData:
        with self.build_client() as client:
            ...
            return LeyesData(laws=self._map_laws(sanctioned, summary_by_law, title_by_project))

    def persist(self, session: Session, data: object) -> int:
        assert isinstance(data, LeyesData)
    ...
    def _map_laws(
        ...
        for record in sanctioned:
            ...
            rows_by_law[law_number] = {
                ...
                "first_half_sanction": _fecha(record.get("PRIMERA_MEDIA_SANCION")),
                "second_half_sanction": _fecha(record.get("SEGUNDA_MEDIA_SANCION")),
                "final_sanction": _fecha(record.get("SANCION_DEFINITIVA")),
                "title": title,
