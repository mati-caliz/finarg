...
const AR_CHART_TYPES = new Set<string>(AR_CURRENCY_GROUPS.usd);
  ...
  const historyQueries = useQueries({
    queries: chartableSelectedTypes.map((type) => ({
      ...
      queryFn: async () => {
        ...
        const from = new Date(Date.now() - Number.parseInt(period) * 24 * 60 * 60 * 1000)
          .toISOString()
          .split("T")[0];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {isLoading
          ? Array.from({ length: currencyTypes.length || 4 }, (_, i) => `skeleton-${i}`).map(
              (key) => <DolarCardSkeleton key={key} />,
            )
        ...
        <CardContent>
          {isLoading ? (
            <div className="space-y-3">
              {Array.from(
                { length: currencyTypes.length || 4 },
                (_, i) => `table-skeleton-${i}`,
              ).map((key) => (
